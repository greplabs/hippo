...
fn extract_image_metadata(path: &Path, metadata: &mut MemoryMetadata) -> Result<()> {
    ...
    if let Ok(exif_reader) = exif::Reader::new().read_from_container(&mut reader) {
        ...
        if let Some(field) = exif_reader.get_field(exif::Tag::DateTimeOriginal, exif::In::PRIMARY) {
            ...
            if let Ok(dt) = chrono::NaiveDateTime::parse_from_str(&date_str.trim_matches('"'), "%Y:%m:%d %H:%M:%S") {
                exif_data.taken_at = Some(chrono::DateTime::from_naive_utc_and_offset(dt, chrono::Utc));
            }
        }
        
        // GPS coordinates
        let lat = extract_gps_coord(&exif_reader, exif::Tag::GPSLatitude, exif::Tag::GPSLatitudeRef);
        let lon = extract_gps_coord(&exif_reader, exif::Tag::GPSLongitude, exif::Tag::GPSLongitudeRef);
...
mod tests {
    ...
    fn test_function_name_extraction() {
        assert_eq!(extract_function_name("fn hello_world() {", "fn "), Some("hello_world".into()));
        assert_eq!(extract_function_name("pub fn process_data(x: i32)", "fn "), Some("process_data".into()));
        assert_eq!(extract_function_name("def calculate_sum(a, b):", "def "), Some("calculate_sum".into()));
    }
}
