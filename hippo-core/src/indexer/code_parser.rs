    ...
    fn parse_rust(&self, content: &str, info: &mut CodeInfo) {
        ...
        for (line_num, line) in content.lines().enumerate() {
               ...
               trimmed.starts_with("async fn ") || trimmed.starts_with("pub async fn ") {
                ...
                if let Some(name) = extract_rust_fn_name(trimmed) {
                    ...
                    info.functions.push(FunctionInfo {
                        name: if is_async { format!("async {}", name) } else { name },
            ...
            if trimmed.starts_with("pub struct ") || trimmed.starts_with("pub enum ") ||
               trimmed.starts_with("pub trait ") || trimmed.starts_with("pub type ") {
                if let Some(name) = extract_name_after(trimmed, &["pub struct ", "pub enum ", "pub trait ", "pub type "]) {
    ...
    fn parse_python(&self, content: &str, info: &mut CodeInfo) {
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            if trimmed.starts_with("import ") {
                ...
                if let Some(pkg) = import.split('.').next().map(|s| s.split(' ').next()).flatten() {
            ...
            if trimmed.starts_with("from ") && trimmed.contains(" import ") {
                ...
                // Extract package name
                if let Some(pkg) = trimmed.strip_prefix("from ").and_then(|s| s.split(' ').next()) {
...
mod tests {
    ...
    fn test_rust_fn_extraction() {
        ...
        assert_eq!(extract_rust_fn_name("pub fn world(x: i32) -> bool {"), Some("world".into()));
        assert_eq!(extract_rust_fn_name("async fn fetch<T>() {"), Some("fetch".into()));
    }
    
    #[test]
    fn test_python_fn_extraction() {
        assert_eq!(extract_python_fn_name("def hello():"), Some("hello".into()));
        assert_eq!(extract_python_fn_name("async def fetch(url):"), Some("fetch".into()));
    }
    
    #[test]
    fn test_js_fn_extraction() {
        assert_eq!(extract_js_fn_name("function hello() {"), Some("hello".into()));
        assert_eq!(extract_js_fn_name("async function fetch() {"), Some("fetch".into()));
    }
}
