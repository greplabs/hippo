...
impl KnowledgeGraph {
    ...
    pub async fn build_mind_map(&self, center_id: MemoryId, depth: usize) -> Result<MindMap> {
        let _center = self.storage.get_memory(center_id).await?
        ...
        while let Some((current_id, current_depth)) = queue.pop_front() {
            ...
            // Add node
            nodes.insert(current_id, MindMapNode {
                id: current_id,
                ...
                size: if current_id == center_id { 1.5 } else { 1.0 - (current_depth as f32 * 0.15) }.max(0.4),
            ...
            if current_depth < 2 {
                let similar = self.storage.find_similar(current_id, 5).await.unwrap_or_default();
                for (similar_id, score) in similar {
                    if similar_id != current_id && !visited.contains(&similar_id) && current_depth < depth {
    ...
    pub async fn get_related(&self, memory_id: MemoryId, limit: usize) -> Result<Vec<Memory>> {
        ...
        let similar = self.storage.find_similar(memory_id, limit * 2).await.unwrap_or_default();
    ...
    pub async fn build_code_graph(&self, root_path: &str) -> Result<CodeGraph> {
        ...
        for memory in &memories {
            if let MemoryKind::Code { language, lines } = &memory.kind {
                ...
                nodes.push(CodeGraphNode {
                    ...
                    lines: *lines,
                    imports: memory.metadata.code_info.as_ref()
                        .map(|c| c.imports.clone())
                        .unwrap_or_default(),
                    exports: memory.metadata.code_info.as_ref()
                        .map(|c| c.exports.clone())
                        .unwrap_or_default(),
                    functions: memory.metadata.code_info.as_ref()
            ...
            if let Some(memory) = memories.iter().find(|m| m.id == node.id) {
                if let Some(code_info) = &memory.metadata.code_info {
                    for import in &code_info.imports {
                        ...
                        if let Some(target_id) = resolve_import(&import, &file_to_id, &node.path, &node.language) {
        ...
        let files_by_module: HashMap<String, Vec<MemoryId>> = nodes.iter()
            .filter_map(|n| {
    ...
    pub async fn suggest_connections(&self, memory_id: MemoryId) -> Result<Vec<(Memory, ConnectionKind, f32)>> {
...
fn compute_radial_layout(nodes: &mut [MindMapNode], center_id: MemoryId) {
    ...
    for node in nodes.iter_mut() {
        if node.id == center_id {
            node.position = Some(Position { x: center_x, y: center_y });
            break;
        }
    }
...
mod tests {
    ...
    fn test_kind_to_color() {
        let img = MemoryKind::Image { width: 100, height: 100, format: "jpg".into() };
